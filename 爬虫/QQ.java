...
public class QQ extends Spider {
    ...
    protected JSONObject f251dn;

    protected JSONObject f252q = new JSONObject();

    private String m1968q(String str, String str2) {
        String str3;
        try {
            if (str2.startsWith("//")) {
            ...
            if (hashMap != null) {
                for (String str4 : hashMap.keySet()) {
                    if (hashMap.get(str4).trim().length() != 0) {
                        str3 = str3 + "&" + str4 + "=" + URLEncoder.encode(hashMap.get(str4).trim());
                    }
                }
            }
            String m1172h = OkHttpUtil.string(str3, headers(str3));
            JSONObject jSONObject = new JSONObject();
            try {
                Elements m1672B1 = Jsoup.parse(m1172h).select(".list_item");
                JSONArray jSONArray = new JSONArray();
                for (int i = 0; i < m1672B1.size(); i++) {
                    Element c0172Hq = m1672B1.get(i);
                    String mo104Pd = c0172Hq.selectFirst("a").attr("title");
                    String m1968q = m1968q(str3, c0172Hq.selectFirst("img").attr("src"));
                    String m1636pE = c0172Hq.selectFirst(".figure_caption") == null ? "" : c0172Hq.selectFirst(".figure_caption").text();
                    String mo104Pd2 = c0172Hq.selectFirst("a").attr("data-float");
                    JSONObject jSONObject2 = new JSONObject();
                    jSONObject2.put("vod_id", mo104Pd2);
                    jSONObject2.put("vod_name", mo104Pd);
                    jSONObject2.put("vod_pic", m1968q);
                    jSONObject2.put("vod_remarks", m1636pE);
    ...
    public String detailContent(List<String> list) {
        ...
        try {
            ...
            JSONObject jSONObject = new JSONObject(OkHttpUtil.string(str, headers(str)));
            ...
            jSONObject2.put("vod_pic", m1968q(str, optJSONObject.optString("pic")));
            jSONObject2.put("type_name", jSONObject.optString("typ"));
            ...
            while (i2 <= jSONArray3.length()) {
                ...
                JSONArray jSONArray4 = new JSONObject(OkHttpUtil.string(str2, headers(str2)).substring(13)).getJSONArray("results");
                int i3 = 0;
    ...
    /* renamed from: dn */
    protected HashMap<String, String> headers(String str) {
    ...
    public String homeContent(boolean z) {
        try {
            Document m1571ue = Jsoup.parse(OkHttpUtil.string("https://v.qq.com/channel/tv?listpage=1&channel=tv&sort=18&_all=1", headers("https://v.qq.com/channel/tv?listpage=1&channel=tv&sort=18&_all=1")));
            JSONArray jSONArray = new JSONArray();
            Iterator<Element> it = m1571ue.select(".nav_cell").iterator();
            while (it.hasNext()) {
                Element next = it.next();
                JSONObject jSONObject = new JSONObject();
                String mo104Pd = next.selectFirst("a").attr("href");
                if (!mo104Pd.contains("/art") && !mo104Pd.contains("feeds_hotspot") && !mo104Pd.contains("wwe") && !mo104Pd.contains("choice") && !mo104Pd.contains("sports_new") && !mo104Pd.contains("games") && !mo104Pd.contains("lols11") && !mo104Pd.contains("ent") && !mo104Pd.contains("news") && !mo104Pd.contains("fashion") && !mo104Pd.contains("tech") && !mo104Pd.contains("auto") && !mo104Pd.contains("house") && !mo104Pd.contains("finance") && !mo104Pd.contains("astro") && !mo104Pd.contains("nba") && !mo104Pd.contains("fun") && !mo104Pd.contains("baby") && !mo104Pd.contains("music") && !mo104Pd.contains("life") && !mo104Pd.contains("travel") && mo104Pd.contains("/channel/")) {
                    jSONObject.put("type_name", next.selectFirst("a").text());
                    jSONObject.put("type_id", mo104Pd.split("/channel/")[1]);
                    jSONArray.put(jSONObject);
                }
            }
            String m1172h = OkHttpUtil.string("https://v.qq.com/x/bu/pagesheet/list?_all=1&append=1&channel=choice", headers("https://v.qq.com/x/bu/pagesheet/list?_all=1&append=1&channel=choice"));
            JSONObject jSONObject2 = new JSONObject();
            if (z) {
                jSONObject2.put("filters", this.f251dn);
            }
            ...
            try {
                ...
                for (int i = 0; i < m1672B1.size(); i++) {
                    Element c0172Hq = m1672B1.get(i);
                    String mo104Pd2 = c0172Hq.selectFirst("a").attr("title");
                    String m1968q = m1968q("https://v.qq.com/x/bu/pagesheet/list?_all=1&append=1&channel=choice", c0172Hq.selectFirst("img").attr("src"));
                    String m1636pE = c0172Hq.selectFirst(".figure_caption").text();
                    String mo104Pd3 = c0172Hq.selectFirst("a").attr("data-float");
                    JSONObject jSONObject3 = new JSONObject();
                    jSONObject3.put("vod_id", mo104Pd3);
                    jSONObject3.put("vod_name", mo104Pd2);
                    jSONObject3.put("vod_pic", m1968q);
                    jSONObject3.put("vod_remarks", m1636pE);
    ...
    public String homeVideoContent() {
        try {
            JSONArray jSONArray = new JSONObject(OkHttpUtil.string("https://v.qq.com/api.php/app/index_video?token=", headers("https://v.qq.com/api.php/app/index_video?token="))).getJSONArray("list");
            JSONArray jSONArray2 = new JSONArray();
    ...
    public String searchContent(String str, boolean z) {
        try {
            ...
            JSONArray jSONArray = new JSONObject(OkHttpUtil.string(str2, headers(str2))).getJSONArray("uiData");
            JSONArray jSONArray2 = new JSONArray();
            for (int i = 0; i < jSONArray.length(); i++) {
